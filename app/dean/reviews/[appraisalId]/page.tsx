...
async function computeAll(appraisalId: number) {
    ...
    await prisma.$transaction([
        prisma.evaluation.upsert({
            ...
            create: { appraisalId, role: 'DEAN', researchPts, universityServicePts, communityServicePts, teachingQualityPts, totalScore },
        }),
...
async function getAppraisalData(appraisalId: number, deanUserId: number) {
    const dean = await prisma.user.findUnique({ where: { id: deanUserId }, include: { department: true } });
    const appraisal = await prisma.appraisal.findUnique({
        where: { id: appraisalId },
        include: {
    ...
    if (!appraisal || appraisal.faculty.role !== 'HOD' || appraisal.faculty.department?.collegeId !== dean?.department?.collegeId || appraisal.facultyId === deanUserId) {
        return null;
    }
    return appraisal;
}
                ...
                <TabsContent value="achievements">
                    <Card>
                        <CardHeader>
                            ...
                            <p className="text-muted-foreground">Read-only view of the faculty member's submitted achievements.</p>
                        </CardHeader>
                        <CardContent>
